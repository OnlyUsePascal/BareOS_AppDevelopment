...
void mbox_setup (unsigned int tagId, unsigned int reqSz, 
                  unsigned int resSz, unsigned int **resData) {
  unsigned int tagBufSz = (resSz >= reqSz) ? resSz : reqSz;

  //round up to nearest multiple of sizeof(uint32)
  tagBufSz = (tagBufSz + MBOX_UNIT_SZ - 1) / MBOX_UNIT_SZ; 
  tagBufSz *= MBOX_UNIT_SZ;
  
  //basic size + tag buffer size
  unsigned int mBufSz = 6 * MBOX_UNIT_SZ + tagBufSz; 

  // attach tag info + prepare address holder for reply data.
  mBuf[0] = mBufSz; 
  mBuf[1] = MBOX_REQUEST;
  mBuf[mBufSz / MBOX_UNIT_SZ - 1] = MBOX_TAG_LAST;
  
  switch (tagId) {
    case MBOX_TAG_GETBOARDREVISION:{
      mBuf[2] = tagId;   
      mBuf[3] = tagBufSz; //buffer size
      mBuf[4] = 0;        //request code
      mBuf[5] = 0;        //clear buffer
      *resData = (unsigned int *) &mBuf[5];
      break;
    }
    case MBOX_TAG_GETMACADDR: {
      mBuf[2] = tagId;
      mBuf[3] = tagBufSz; // buffer sz
      mBuf[4] = 0;        // request code
      mBuf[5] = 0;        // clear buffer
      mBuf[6] = 0;        // clear buffer
      *resData = (unsigned int *) &mBuf[5];
      break;
    }
    default:
      uart_puts("Tag ID not found!\n");
      break;
  }
}
