...
void game_enter(){
  ...
  int actionsSz = 4;
  for (int i = 0; i < 4; i++){
    font_drawString(250, posY + i * offsetY, actions[i], GAME_MENU_FOREGND, 2);
  }
  
  // menu option choose (start, continue, help, exit)
  int actionIdx = 0; 
  font_drawChar(200, posY, '>', GAME_MENU_FOREGND, 2);

  while (1){ 
    char c = uart_getc();
    uart_sendc(c); uart_sendc('\n');
    if (c == 'w' || c == 'a') {
      font_drawChar(200, posY + actionIdx * offsetY, '>', GAME_MENU_BACKGND, 2);
      actionIdx = (actionIdx - 1) % actionsSz;
      if (actionIdx < 0) actionIdx += actionsSz;
      font_drawChar(200, posY + actionIdx * offsetY, '>', GAME_MENU_FOREGND, 2);
    } else if (c == 's' || c == 'd') {
      font_drawChar(200, posY + actionIdx * offsetY, '>', GAME_MENU_BACKGND, 2);
      actionIdx = (actionIdx + 1) % actionsSz;
      font_drawChar(200, posY + actionIdx * offsetY, '>', GAME_MENU_FOREGND, 2);
